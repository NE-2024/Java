...
import rw.ac.rca.studentCourse.v1.dto.responses.LoginResponse;
import rw.ac.rca.studentCourse.v1.dto.responses.UserDTOMapper;
...
import rw.ac.rca.studentCourse.v1.security.jwt.JwtUtils;
import rw.ac.rca.studentCourse.v1.security.user.UserAuthority;
...
import rw.ac.rca.studentCourse.v1.services.UserService;
import rw.ac.rca.studentCourse.v1.utils.Hash;
...
public class UserServiceImpl implements UserService {
    ...
    public ResponseEntity<ApiResponse> createUser(CreateUserDTO createUserDTO) throws Exception{
         ...
         if(!user1.isPresent()){
             ...
             Role roleOptional = roleRepository.findById(1L).orElseThrow(()->new ResourceNotFoundException("id", "Role", "1"));
    ...
    @PreAuthorize("hasAuthority('Admin') or #user_id == authentication.principal.grantedAuthorities[0].userId")
    public ResponseEntity<ApiResponse> deleteUser(UUID user_id) throws Exception{
        if (userRepository.existsById(user_id)) {
            Optional<User> user = userRepository.findById(user_id);
    ...
    public ResponseEntity<ApiResponse> updatePassword(UUID user_id , String newPassword) throws Exception{
        try {
